...
public class FixedWidthLongListLabel extends AbstractLongListLabel {
	 ...
	 */
	public FixedWidthLongListLabel(final String key, final int width, final long[] value) {
		super(key, value);
	 ...
	 * @param width the label width (in bits).
	 */
	public FixedWidthLongListLabel(final String key, final int width) {
		this(key, width, LongArrays.EMPTY_ARRAY);
	}

	/** Creates a new fixed-width longs label using the given key and width
	 *  with an empty list.
	 *
	 * @param arg two strings containing the key and the width of this label.
	 */
	public FixedWidthLongListLabel(final String... arg) {
	...
	public int toBitStream(final OutputBitStream outputBitStream, final int sourceUnused) throws IOException {
		...
		for (final long element : value) bits += outputBitStream.writeLong(element, width);
		return bits;
	}
